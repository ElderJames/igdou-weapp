...
const DEFAULT = {
    ...
    lineAnimationViewName: "lineAnimationData",
    /**
     * 事件回调
   * @type {[type]}
    */
   ...
   * @returns {*}
    */
    ...
    slidePrev(runCallbacks = false, speed = 300) {
        const self = this;
        const { onSlidePrevStart, onSlidePrevEnd } = self;

        typeof onSlidePrevStart === "function" && onSlidePrevStart(self); // slide达到过渡条件时执行

        self.slideTo(self.activeIndex - 1, speed, runCallbacks);

        typeof onSlidePrevEnd === "function" &&
            setTimeout(() => {
                onSlidePrevEnd(self);
            }, speed); //  slide过渡结束后执行
    }
    ...
    slideBack(runCallbacks = false, speed = 300) {
        const self = this;
        const { XORY, activeIndex, rectDistance, onTransitionEnd } = self;

        const translate = -activeIndex * rectDistance;

        self.slideAnimation(translate, speed);

        typeof onTransitionEnd === "function" &&
            setTimeout(() => {
    ...
    slideTo(index, speed = 300, runCallbacks = false) {
        ...
        try {
            ...
            this.activeTab = self.activeIndex;

            runCallbacks &&
                typeof onSlideChangeStart === "function" &&
                onSlideChangeStart(self); // slide达到过渡条件时执行

            self.slideAnimation(translate, speed);

            runCallbacks &&
                typeof onSlideChangeEnd === "function" &&
                setTimeout(() => {
    ...
    slideAnimation(translate = 0, speed = 300, timingFunction = "ease") {
        ...
        console.log({ translate, speed, timingFunction })
        ...
        try {
            ...
            // tab定位线的动画
            const lineAnimation = wx.createAnimation({
                transformOrigin: "50% 50%",
                duration: speed,
                timingFunction,
                delay: 0
            ...
            if (XORY == "X") {
                lineAnimation[`translate${XORY}`](-translate / this.slideLength).step();
                this.syncView(lineAnimationViewName, lineAnimation);
            }
        } catch (err) {
    ...
    initSwiper(param) {
        ...
        const directionClass =
            direction === "horizontal"
                ? "we-container-horizontal"
                : "we-container-vertical";
        ...
        this.slideLength = tabTiles.length;

        this.tabWidth = this.width / tabTiles.length;
